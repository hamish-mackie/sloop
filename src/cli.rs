...
pub enum Command {
    ...
    #[command(hide = true)]
    Ready { ticket: String },
    /// List tickets and why they are not running.
    List,
    /// Show daemon state.
    Status,
...
fn run_command(
    ...
    match command {
        ...
        | Command::Resume
        | Command::Cancel { .. }
        | Command::Logs { .. }) => match Request::try_from(command) {
        ...
        command => match Request::try_from(command) {
            Ok(request) => write_response(
                mode,
                Some(request.verb()),
                stdout,
                &ResponseEnvelope::success(
                    None,
                    json!({"verb": request.verb(), "implemented": false}),
                ),
                ExitCode::SUCCESS,
            ),
            Err(error) => write_cli_error(mode, stderr, error.to_string()),
...
mod tests {
    ...
    #[test]
    fn response_envelope_stays_valid_json_with_the_json_flag() {
        let mut stdout = Vec::new();
        let mut stderr = Vec::new();
        let status = super::run(["sloop", "--json", "reindex"], &mut stdout, &mut stderr);

        assert_eq!(status, std::process::ExitCode::SUCCESS);
        let response: Value = serde_json::from_slice(&stdout).unwrap();
        assert_eq!(response["ok"], true);
        assert_eq!(response["data"]["verb"], "reindex");
        assert!(stderr.is_empty());
    }

    #[test]
    fn output_is_human_text_by_default() {
        let mut stdout = Vec::new();
        let mut stderr = Vec::new();
        let status = super::run(["sloop", "reindex"], &mut stdout, &mut stderr);

        assert_eq!(status, std::process::ExitCode::SUCCESS);
        let text = String::from_utf8(stdout).unwrap();
        assert!(serde_json::from_str::<Value>(&text).is_err(), "{text}");
        assert!(text.contains("reindex"), "{text}");
        assert!(stderr.is_empty());
    }
}
