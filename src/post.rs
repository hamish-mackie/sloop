...
pub fn handle(
    ...
    if stamped.name.trim().is_empty() {
        return Err(PostError::MissingName { path: relative_str });
    }
    if !stamped.has_blocked_by() {
        return Err(PostError::MissingBlockedBy { path: relative_str });
    }
    if frontmatter::body(&content)
        ...
        .is_empty()
    {
        return Err(PostError::EmptyBody { path: relative_str });
    }
    ...
    let activation = match &args.activation {
        ...
        PostActivation::At { time } => {
            let mut activation = queue_activation(store, &ticket.id, ActivationKind::At, now_ms)?;
            activation["time"] = json!(time);
            activation
        }
    };
...
/// enqueue duplicate work. Eligible times for `at` activations are computed
/// by the scheduler when time gates land; posting only records intent.
fn queue_activation(
    store: &Store,
    ticket_id: &str,
    kind: ActivationKind,
