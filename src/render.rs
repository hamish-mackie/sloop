...
fn render_list(data: &Value) -> String {
    ...
    let state_width = tickets
        ...
        .max()
        .unwrap_or(1);
    let project_width = tickets
        .iter()
        .filter_map(|ticket| ticket["project"].as_str())
        .map(str::len)
        .max()
        .unwrap_or(1);
    ...
    for ticket in tickets {
        ...
        let _ = write!(text, "{id:id_width$}  {state:state_width$}  ({project})",);
        let terminal = matches!(state, "merged" | "needs_review");
        if ticket["run"].is_null()
            && !terminal
            && let Some(reason) = ticket["reason"].as_str()
        {
            text.push_str(&" ".repeat(project_width.saturating_sub(project.len())));
...
fn render_logs(data: &Value) -> String {
    ...
    for entry in entries {
        ...
        let _ = writeln!(text, "[{origin}] {line}");
    }
    text
}
...
mod tests {
    ...
    fn list_renders_reasons_but_not_running_or_terminal_reasons() {
        let response = ResponseEnvelope::success(
            ...
            json!({
                "tickets": [
                    {"id": "TICKET-1", "project": "default", "state": "ready", "run": null,
                     "reason": "scheduler is paused; resume with `sloop resume`"},
                    {"id": "T2", "project": "default", "state": "claimed", "run": "R1",
                     "reason": "claimed by run R1"},
                    {"id": "T3", "project": "web", "state": "merged", "run": null,
                     "reason": null}
                ]
            }),
        );
        ...
        assert_eq!(
            ...
            "TICKET-1  ready    (default)  — scheduler is paused; resume with `sloop resume`\n\
             T2        claimed  (default)\n\
             T3        merged   (web)\n"
        );
    }
    ...
    #[test]
    fn genuinely_unimplemented_stubs_say_so_in_plain_words() {
        let response =
            ResponseEnvelope::success(None, json!({"verb": "reindex", "implemented": false}));
        let text = render(Some("reindex"), &response);
        assert_eq!(text, "reindex is not implemented by the daemon yet\n");
    }

    #[test]
    fn log_entries_render_origin_and_text() {
        let response = ResponseEnvelope::success(
            None,
            json!({
                "run": "R1",
                "entries": [
                    {"source": "agent", "stage": null, "text": "hello"},
                    {"source": "aftercare", "stage": "test", "bytes_b64": "AAE="}
                ],
                "next_cursor": 2,
                "complete": true
            }),
        ...
        assert!(text.contains("[agent] hello"), "{text}");
        assert!(text.contains("[aftercare:test] <binary output>"), "{text}");
    }
}
