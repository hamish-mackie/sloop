...
impl Store {
        ...
        let ticket = self
            ...
            .query_row(
                "SELECT t.id FROM tickets t
                 ...
                 LIMIT 1",
                params![activation.project_id, activation.id],
                |row| row.get(0),
            )
            .optional()?;
        Ok(ticket)
    ...
    /// Returns the next ordinal for allocating a run ID; runs are never
    /// deleted and only the dispatcher allocates.
    ...
    pub(crate) fn recoverable_runs(&self) -> Result<Vec<RecoverableRun>, StoreError> {
        let mut statement = self.connection.prepare(
            "SELECT r.id, r.ticket_id, r.state, r.branch, r.worktree_path,
    ...
    /// Returns the next ordinal for allocating an activation ID. Only the
    /// single-threaded dispatcher allocates IDs, so count-plus-one cannot
    /// race with itself, and activations are never deleted.
...
mod tests {
    ...
    fn ready_work_selection_is_deterministic_and_respects_filters() {
        ...
        assert_eq!(store.select_ready_ticket(&scoped).unwrap(), None);
    }
