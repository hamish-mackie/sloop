...
Sloop runs each ticket in its own Git worktree, reviews the result, and merges
it when it passes.
...
- **Agent orchestration** — each ticket picks its agent, model, and effort:
  ...
  workers.
- **Agentic loops** — combine predefined flows, parallel agents, and scheduled
  execution.
...
Under the hood it's a small Rust daemon that judges outcomes from commits and
tests.
