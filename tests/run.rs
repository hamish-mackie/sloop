...
fn list_explains_paused_failed_held_and_claimed_tickets() {
    ...
    let human = human
        ...
        .collect::<Vec<_>>()
        .join("\n");
    assert!(human.contains(&format!("{paused} ready (default) — scheduler is paused")));
    assert!(human.contains(&format!(
        "{failed} failed (default) — failed after 1 attempt(s)"
    )));
    assert!(human.contains(&format!("{held} held (default) — held by operator")));
    assert!(human.contains(&format!("{claimed} claimed (default)")));
}
